...
import styles from "./InvoicePaidPage.module.css"
import { useEffect, useState } from "react"
import api from "../../api";



const InvoicePaidPage = ({setNumberCartItems}) => {
  ...
  useEffect(function(){
    ...
    if(status && ref) {
      api.post(`completed_page/?ref=${ref}&status=${status}`)
      .then(res => {
    ...
    <header className="py-3 my-5" style={{backgroundColor: "#3A6B35"}}>
        <div className="container px-4 px-lg-5 my-5">
            <div className="text-center text-white">
                ...
                <Link onClick={removeCart} to="/" className={`${styles.pill_button} btn btn-light btn-lg rounded-pill px-4 py-2`}>Back Home</Link>
            </div>
...
export default InvoicePaidPage